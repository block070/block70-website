...
import { TOTAL_COINS_PAGINATED } from "@/lib/coins";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        if (list.length > 0) {
          setCoins(apiCoinsToShape(list, page, limit));
        } else {
        ...
        try {
          ...
          const chunk = await getMarketCoins({ limit: marketPerPage, page });
          if (cancelled) return;
          if (chunk.length > 0) {
            setCoins(marketCoinsToShape(chunk, page, limit));
          }
        } catch (e) {
          if (!cancelled) {
