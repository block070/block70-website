...
export default function StrategiesTopPage() {
  const [entries, setEntries] = useState<LeaderboardEntry[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getStrategyLeaderboard(20, true)
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-slate-50">Top strategies</h1>
      </div>

      <Card>
        <CardHeader
          title="Ranked by performance (ROI × win rate)"
          subtitle="Public strategies only"
        />
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <p className="p-4 text-slate-500">
              No public strategies with backtests yet.
            </p>
          ) : (
                ...
                {entries.map((e) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2">
                      <Link href={`/strategies/${e.strategy_id}`}>
