...
export default async function WalletChainAddressPage({ params }: Props) {
  ...
  const netflowStr =
    live.fetchError
      ...
      : netflow == null
        ? "--"
        : `${netflow >= 0 ? "+" : ""}${formatCoin(seed.chain, netflow)}`;
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3">
          ...
          <p className="text-lg font-semibold text-slate-100">
            {live.fetchError ? live.fetchError : formatCoin(seed.chain, live.balance)}
          </p>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3">
          <p className="text-[11px] text-slate-500">Tx Count (24h)</p>
          <p className="text-lg font-semibold text-slate-100">
            {live.fetchError ? live.fetchError : live.txCount == null ? "--" : live.txCount}
          </p>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-900/50 p-3">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
        ...
        <p className="mt-2 text-xs text-slate-400">
          {live.fetchError
            ? live.fetchError
            : live.lastActivity
              ? new Date(live.lastActivity).toLocaleString()
              : "--"}
        </p>
        <div className="mt-3 text-xs text-slate-400">
          <span className="font-medium text-slate-200">Inflow 24h:</span>{" "}
          {live.fetchError ? live.fetchError : formatCoin(seed.chain, live.inflow24h)}
          <br />
          <span className="font-medium text-slate-200">Outflow 24h:</span>{" "}
          {live.fetchError ? live.fetchError : formatCoin(seed.chain, live.outflow24h)}
        </div>
      </section>
    </div>
  );
