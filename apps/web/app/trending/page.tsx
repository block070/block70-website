...
async function fetchTrending(): Promise<TrendingCoin[]> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL ?? ""}/api/v1/market/trending?limit=20`,
    {
      cache: "no-store",
    },
  );
  if (!res.ok) {
    throw new Error("Failed to load trending coins");
  }
  return (await res.json()) as TrendingCoin[];
}
    ...
    <div className="space-y-6">
      ...
      {error ? (
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-3 text-xs text-slate-400">
          No trending data from CoinGecko yet. Try refreshing in a few
          seconds.
        </div>
      ) : (
    ...
    <Suspense
      fallback={
        <div className="space-y-6">
          ...
          <div className="h-40 rounded-xl border border-slate-800 bg-slate-950/60" />
        </div>
      }
    >
      <TrendingContent />
