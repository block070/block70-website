...
export async function fetchCoingeckoGlobal(): Promise<CoingeckoGlobalNumbers | null> {
  ...
  for (let a = 0; a < attempts; a++) {
    try {
      ...
      if (!res.ok) {
        if (a < attempts - 1) await new Promise((r) => setTimeout(r, 350 * (a + 1)));
        continue;
      }
      ...
      const out: CoingeckoGlobalNumbers = {
      ...
      if (
        out.total_market_cap_usd != null ||
        out.total_volume_usd != null ||
        out.btc_dominance_pct != null ||
        out.eth_dominance_pct != null
      ) {
        return out;
      }
      if (a < attempts - 1) await new Promise((r) => setTimeout(r, 350 * (a + 1)));
...
export async function fetchCoingeckoMarketsTop(perPage = 100): Promise<MarketCoin[]> {
  ...
  for (let a = 0; a < attempts; a++) {
    try {
      ...
      if (!res.ok) {
        if (a < attempts - 1) await new Promise((r) => setTimeout(r, 400));
        continue;
      }
