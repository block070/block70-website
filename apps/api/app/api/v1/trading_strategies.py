...
def create_strategy(
    ...
    strategy = TradingStrategy(
        ...
        conditions_json=_conditions_to_str(payload.conditions_json),
...
def get_strategy_backtest(
    ...
    return StrategyBacktestRead.model_validate(latest)
...
def leaderboard(
    ...
    scored.sort(key=lambda x: (x[1].average_profit or 0) * (x[1].win_rate or 0), reverse=True)
    out = []
        ...
        "templates": [
            {
                ...
                "description": "Enter when smart wallets accumulate the token",
                "conditions_json": {"min_confidence": 0.7, "signal_types": ["wallet"]},
            },
            {
                "id": "signal_breakout",
                "name": "Signal breakout",
                "description": "Enter on strong signal cluster breakout",
                "conditions_json": {"min_signal_strength": 0.8, "min_confidence": 0.75},
            },
            {
                "id": "whale_tracking",
                "name": "Whale tracking",
                "description": "Follow large wallet accumulation",
                "conditions_json": {"signal_types": ["wallet"], "min_signal_strength": 0.7},
            },
        ]
    }
